    ...
    for data_source, prompt2var2vals in data_src2prompt2var2vals.items():
        for prompt, var2vals in prompt2var2vals.items():
            for var_name, var_vals in var2vals.items():
                ...
                filtered_pairs = [(val, pred) for val, pred in zip(var_vals, var2vals.get("pred", [None] * len(var_vals))) if val is not None]
                if not filtered_pairs:
                    continue

                filtered_vals = [val for val, _ in filtered_pairs]
                ...
                if n_resps > 1:
                    ...
                    for n in ns:
                        ...
                        if var2vals.get("pred", None) is not None:
                            vote_data = [{"val": val, "pred": pred} for val, pred in filtered_pairs if pred is not None]
